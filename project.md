...
- ⚡ **Streamlit DOMPurify injects `target="_blank"` on all `<a>` tags:** even with `target="_self"` explicitly set, DOMPurify may strip or override it. Belt-and-suspenders fix: set `target="_self"` in Python-generated HTML AND inject a `MutationObserver` script into Streamlit's `index.html` (via `_patch_streamlit_index()`) that watches the DOM and flips any `target="_blank"` back to `_self` for app navigation links (`?item=`, `?building=`, `?phase=`).
...
14. ⚡ ☑ <span style="color: purple">**Fix links opening new tabs**</span> — Streamlit's DOMPurify injects `target="_blank"` on all `<a>` tags. Belt-and-suspenders fix: (1) added `target="_self"` to all 4 link sites in `app.py` (recipe ingredient/product chips, machine link, building cost chips, building produces chips); (2) extended `_patch_streamlit_index()` to inject a `<script>` MutationObserver into Streamlit's `index.html` that watches for any `<a target="_blank">` whose href matches `?item=`, `?building=`, or `?phase=` and flips to `_self`. Committed `a75b994`.
